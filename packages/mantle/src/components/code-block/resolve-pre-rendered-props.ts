...
/**
 * Parses a code block metastring (e.g. `title="hello" mode=cli collapsible`) into a `Meta` object.
 */
...
/**
 * Normalizes a value from a metastring token, stripping surrounding double quotes if present.
 */
...
/**
 * Tokenizes a metastring into key or key=value tokens, respecting quoted values.
 */
...
/**
 * Extracts and normalizes the Mantle pre-rendered code block payload from HTML element props.
 *
 * Returns `mantleCode: undefined` if no mantle payload is present (plain `<pre>` passthrough).
 * Otherwise returns the normalized payload and remaining non-mantle props.
 */
