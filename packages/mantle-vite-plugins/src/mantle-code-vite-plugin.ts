...
import { highlightWithMantleShiki } from "@ngrok/mantle-code-block-tools/server-highlighter";
...
/**
 * Vite plugin that transforms `mantleCode("lang")\`...\`` tagged templates at build time,
 * inlining pre-rendered Shiki HTML into the module so no highlighting work happens in the browser.
 *
 * Include in your `vite.config.ts` via `mantleCodeBlockPlugins()`.
 */
