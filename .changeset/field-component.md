...
Add `Field`, a compound layout primitive for a single form field. Pair with the existing mantle `<Label>` — the component intentionally does not invent a new label primitive. Layout and supplemental parts support `asChild`; `Field.Set`, `Field.Legend`, and `Field.Error` always render `<fieldset>`, `<legend>`, and `<li>` respectively so their core semantics cannot be accidentally removed.
...
- `Field.ErrorList` / `Field.Error` — semantic `<ul role="list">` of `<li>` validation errors. `Field.ErrorList` renders nothing when given no children, so it's safe to leave mounted while a validator produces a (possibly empty) error array.
...
`Field.Item` auto-generates stable IDs for descendant `Field.Description` / non-empty `Field.ErrorList` parts, then merges those IDs onto known form controls via `aria-describedby` / `aria-errormessage` without changing the control's own `id` or `name`, so `id={field.name}` remains the right TanStack Form pattern.
