...
const passwordInputVariants = cva(
	"flex h-11 w-full rounded-md border bg-white px-3 py-2 focus-within:outline-none focus-within:ring-4 disabled:pointer-events-none disabled:opacity-50 dark:bg-gray-50 sm:h-9 sm:text-sm",
	{
		variants: {
			state: {
				default:
					"text-strong border-form placeholder:text-placeholder focus-within:border-blue-600 focus-within:ring-focus-blue",
				danger: "border-red-600 focus-within:border-red-600 focus-within:ring-focus-red",
			},
		},
		defaultVariants: {
			state: "default",
	...
	return (
		<div className={cx(passwordInputVariants({ state }), className)} style={style}>
			...
			<button
				type="button"
				tabIndex={-1}
				className="ml-1 cursor-pointer bg-inherit p-0 text-neutral hover:text-strong"
				onClick={() => {
					setShowPassword((s) => !s);
				}}
			>
