...
3. **Factor the query into a hook.** `useUserQuery` owns fetching, retry, stale-time, and cancellation. The view consumes a `UseQueryResult` and never reaches into TanStack Query directly.
