...
| [`/api/components.json`](/api/components.json)     | Structured component manifest. Each entry has `name`, `slug`, `status`, `importPath`, `docsUrl`, `markdownUrl`, a one-line `summary`, and (where extracted) structured `props` — per-prop `type`, `required`, `default`, `description`, `@enum` member meanings, and `@see` links. |
...
| [`/api/tokens.json`](/api/tokens.json)             | Tailwind 4 semantic design tokens. Each entry has `name`, `cssVar`, `category`, optional `semanticRole`/`aliasOf`, and authored per-theme `values`.                                                                                                                                |
