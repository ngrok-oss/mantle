...
never toggles it; and the `Content` spans full-width below. Override the defaults (`space-y-*` and `divide-y-0` on
`Root` to swap the dividers for spacing; `w-auto` on `Trigger` so it sizes to its label; and `data-state-open:pb-4` on
`Trigger` to keep its padding symmetric so the header row doesn't shift up when the section opens) for the spaced-card
look.
...
<Example className="flex-col gap-6">
	<Accordion.Root
		...
		className="space-y-2.5 divide-y-0"
	>
		<Accordion.Item value="on_tcp_connect">
			<div className="mx-4 flex items-center gap-2">
				<Accordion.Trigger className="w-auto gap-1.5 data-state-open:pb-4">
