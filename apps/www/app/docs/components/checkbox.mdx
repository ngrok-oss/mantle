...
`Choice` works standalone, inside a `Field.Control` (the field's `id` / `name` / `aria-*` flow onto the checkbox and its `aria-describedby` is merged), and in fully-clickable list rows. See the [`Choice` docs](/components/choice) for those variants and the full API.
