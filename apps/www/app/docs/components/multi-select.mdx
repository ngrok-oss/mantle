...
function Example() {
	...
	return (
		<div className="flex w-full max-w-lg flex-col gap-1.5">
			...
			>
				<MultiSelect.Trigger>
					<MultiSelect.TagValues placeholder="Select regions..." />
...
Also accepts all standard `span` element props (except `children` and `onKeyDown`).
...
| `locked?`   | `boolean`    | `false` | When true, disables the remove button and ignores Delete/Backspace key presses.                                           |
...
Renders a sticky footer inside `MultiSelect.Content`. Automatically sticks to the bottom when the popover opens downward and to the top when it flips upward (e.g., near the bottom of the viewport). Place it as the last child of `MultiSelect.Content`.
