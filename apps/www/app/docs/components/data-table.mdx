...
Compose three parts: `DataTable.ExpandHeader` (the toggle column header), `DataTable.RowExpandButton` (the accessible `+`/`−` toggle, dropped into a leading `columnHelper.display` cell), and `DataTable.ExpandedRow` (the detail `<tr>`). Render the `ExpandedRow` directly after `DataTable.Row`, wrapped in a `Fragment` — **never** a DOM element, since a node between `<tbody>` and `<tr>` is invalid HTML and breaks the table's row styling. The detail cell spans every visible column and sits on an opaque surface, so it coexists with a sticky [action column](#action-column-with-a-dropdown-menu).
...
function ExpandableTable({ data }: { data: Payment[] }) {
	...
	return (
		<DataTable.Root table={table}>
			...
			<DataTable.Body>
				{table.getRowModel().rows.map((row) => (
					// Fragment, not a DOM wrapper — a node between <tbody> and <tr> is invalid HTML.
					<Fragment key={row.id}>
