...
`MantleStylesheets` renders the three `<link rel="stylesheet">` tags for the dark, light-high-contrast, and dark-high-contrast theme CSS files. Each stylesheet is gated behind a `media` attribute matching its OS preference so it is non-render-blocking for users who do not need it. Pass `ssrCookie` (from `extractThemeCookie`) so the server can render the correct `media` attribute for users with a manually-selected theme, avoiding FOUC without relying on JavaScript.
...
| `ssrCookie?`  | `string`        |         | The extracted theme cookie from the incoming HTTP request (via `extractThemeCookie`). When provided, the server renders the correct `media` attribute directly in HTML, eliminating the need for the inline fix script for users with a stored non-system theme. |
