...
| `forceTheme?`             | `ResolvedTheme` |         | Force a specific theme's stylesheet to load unconditionally (`media="all"`). Use for pages locked to a single theme (e.g. a dark-only landing page). Only that theme's `<link>` tag is rendered; the others are omitted.                      |
