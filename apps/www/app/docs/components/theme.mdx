...
| `ssrCookie?` | `string` |         | Raw `Cookie` header string from the incoming request. Pass this during SSR so the server can read the persisted theme and render the correct class on `<html>`. |
