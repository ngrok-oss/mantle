...
<!-- 4. Media fix: runs after the <link> tags, reads data-applied-theme from <html> and
        ...
        Generated by: fixMediaScriptContent(undefined) from "@ngrok/mantle/theme" -->
<script nonce="...">
	(function D(e) {
		let {
				darkLinkId: t,
...
> [!NOTE]
> These script strings are generated by `preventWrongThemeFlashScriptContent()` and `fixMediaScriptContent(undefined)` from `@ngrok/mantle/theme`. Re-run the generator after upgrading mantle to pick up any changes.
...
const scriptContent = fixMediaScriptContent(undefined);
...
| Parameter    | Type                         | Description                                                                                                                                                        |
| ------------ | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `forceTheme` | `ResolvedTheme \| undefined` | When set, the script forces that theme's stylesheet to `media="all"`. When `undefined`, the script reads `html[data-applied-theme]` to determine the active theme. |
