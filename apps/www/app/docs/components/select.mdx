...
Set `validation` on `Select.Root` when the whole select has an explicit state; this root-level state is forwarded to the trigger and takes precedence over field-level validation from `Field.Item` / `Field.Control`.
