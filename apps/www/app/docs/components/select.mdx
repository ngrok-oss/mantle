...
When a `Select` is rendered inside `Field.Item`, wrap `Select.Trigger` in `Field.Control`. The trigger is the focusable button, so that is where `Field.Control` applies generated `aria-describedby`, `aria-errormessage`, and validation props.

<Example className="flex-col gap-4">
	<Field.Item className="max-w-64">
		<Field.Label htmlFor="fruits">Fruits</Field.Label>
...
Set `validation` on `Select.Root` when the whole select has an explicit state; this root-level state is forwarded to the trigger and takes precedence over ambient field-level validation from `Field.Item` / `Field.Control`.

Rendered `Field.Errors` or `Field.ErrorList` still force the trigger into the error state because `Field.Control` sets `aria-invalid="true"` on the focusable trigger, and an explicit invalid ARIA value always wins. If a non-error `Select.Root` state needs to win even when errors are rendered, pass `validation` on `Field.Item` or `Field.Control` to suppress the inferred error.
...
When composing with `Field.Item`, wrap `Select.Trigger` in `Field.Control` so helper text and errors are associated with the focusable button. Validation resolves from explicit `Select.Root`, then `Select.Trigger`, then the nearest `Field.Control` / `Field.Item`.
