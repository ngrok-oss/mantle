...
export const loader = async ({ request }: Route.LoaderArgs) => {
	// Pass the Cookie header to useInitialHtmlThemeProps so the server
	// renders the correct theme class, preventing a flash on hydration.
...
export function Layout({ children }: PropsWithChildren) {
	...
	return (
		...
		// theme script that corrects the class before paint on prerendered pages.
